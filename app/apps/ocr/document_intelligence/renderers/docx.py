...
from ..structure.header_footer import PromotedRegion, detect_header_footer_regions
from .ooxml_helpers import add_field_run
...
def render_docx(ast: DocumentAST, pdf_data: bytes | None = None) -> BytesIO:
    ...
    header_regions, footer_regions = detect_header_footer_regions(ast)
    if header_regions:
        _set_promoted_region(doc.sections[0].header, header_regions[0])
    for footer_region in footer_regions:
        _set_promoted_region(doc.sections[0].footer, footer_region)
...
def _set_promoted_region(section_part: object, region: PromotedRegion) -> None:
    """Write a promoted header/footer region with real PAGE/NUMPAGES fields."""
    section_part.is_linked_to_previous = False
    paragraph = (
        section_part.paragraphs[0]
        if section_part.paragraphs
        else section_part.add_paragraph()
    )
    paragraph.text = ""
    for segment in region.segments:
        if segment.kind == "text":
            paragraph.add_run(segment.text)
        elif segment.kind == "page":
            add_field_run(paragraph, "PAGE", segment.text)
        elif segment.kind == "numpages":
            add_field_run(paragraph, "NUMPAGES", segment.text)
...
def _render_textish_node(
    doc: Document, node: object, page_width_px: float
) -> bool:
    ...
    if node.type == LayoutType.heading:
        ...
        return True
    if node.type in (LayoutType.header, LayoutType.footer, LayoutType.page_number):
        return True  # promoted to Word header/footer
...
def _render_node(
    ...
    if _render_textish_node(doc, node, page_width_px):
        return
...
def _add_formula(doc: Document, latex: str) -> None:
    ...
    try:
        ...
        oMathPara = parse_xml(f"<m:oMathPara {nsdecls('m')}>{omath_xml}</m:oMathPara>")  # ruff: ignore[non-lowercase-variable-in-function]
