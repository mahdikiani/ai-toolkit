...
class TestExecutionsRoutes:
    """Integration tests for /executions endpoints."""

    async def test_list_executions_returns_paginated_response(
        self, client: httpx.AsyncClient
    ) -> None:
        """GET /executions should return paginated response structure."""
    ...
    async def test_list_executions_with_pagination_params(
        ...
        """GET /executions should accept pagination parameters."""
        response = await client.get("/executions?offset=0&limit=10")
        assert response.status_code in (200, 401, 403)

    async def test_create_execution_without_auth_returns_401(
        self, client: httpx.AsyncClient
    ) -> None:
        """POST /executions/{prompt_name} without auth should return 401."""
        response = await client.post(
            "/executions/test_prompt",
            json={"input_variables": {"text": "hello"}},
        )
        assert response.status_code in (401, 403)

    async def test_get_execution_not_found(self, client: httpx.AsyncClient) -> None:
        """GET /executions/{uid} for non-existent task should return 401 or 404."""
        response = await client.get("/executions/nonexistent_uid_12345")
        assert response.status_code in (401, 403, 404)

    async def test_create_execution_with_missing_prompt(
        self, authenticated_client: httpx.AsyncClient, tmp_path: Path
    ) -> None:
        """POST /executions/{prompt_name} with missing prompt should return 404."""
...
@pytest.mark.integration
class TestExecutionsPagination:
    """Tests for pagination in execution endpoints."""

    async def test_pagination_response_structure(
        self, client: httpx.AsyncClient
    ) -> None:
        """Paginated responses should have items, total, offset, limit fields."""
        response = await client.get("/executions?offset=0&limit=5")
