...
def test_database_initialized(db: None) -> None:
...
def test_beanie_auto_discovery() -> None:
    """
    ...
    document_models = [
        ...
        if not (
            ...
            and getattr(cls.__dict__["Settings"], "__abstract__", False)
        )
    ]
    ...
    for model in document_models:
        if "Settings" in model.__dict__:
            assert not getattr(model.__dict__["Settings"], "__abstract__", False)


@pytest.mark.asyncio
async def test_mongo_client_server_info(
    mongo_client: AsyncIOMotorClient[dict[str, object]],
) -> None:
    """
