...
class TestBuildAst:
    ...
    def test_orders_nodes_by_reading_order(self) -> None:
        ...
        ordered = [
            _layout_elem("e2", LayoutType.paragraph),
            _layout_elem("e1", LayoutType.paragraph),
        ]
...
class TestBuildDocumentAst:
    ...
    def test_combines_all_pages_in_order(self) -> None:
        ...
        page1 = build_ast(
            [e1], [_layout_elem("e1", LayoutType.paragraph)], page_number=1
        )
        page2 = build_ast(
            [e2], [_layout_elem("e2", LayoutType.paragraph)], page_number=2
        )
    ...
    def test_title_pulled_from_first_title_node(self) -> None:
        title_elem = ProcessedElement(
            id="t1", type=LayoutType.title, text="  My Document  "
        )
        page1 = build_ast(
            [title_elem], [_layout_elem("t1", LayoutType.title)], page_number=1
        )
